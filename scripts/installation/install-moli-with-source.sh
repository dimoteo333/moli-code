...
# Moli Code Installation Script
# This script installs Node.js (via NVM) and Moli Code CLI
# Supports Linux and macOS
#
# Usage: install-moli-with-source.sh --source [github|npm|internal|local-build]
#        install-moli-with-source.sh -s [github|npm|internal|local-build]
...
if [ -z "${BASH_VERSION}" ] && [ -z "${__MOLI_INSTALL_REEXEC:-}" ]; then
    # Check if we're in a git hook environment
    case "${0}" in
        *.git/hooks/*) export __MOLI_IN_GIT_HOOK=1 ;;
    esac
    if [ -n "${GIT_DIR:-}" ]; then
        export __MOLI_IN_GIT_HOOK=1
    fi

    # Try to find bash
    if command -v bash >/dev/null 2>&1; then
        export __MOLI_INSTALL_REEXEC=1
...
# ============================================
echo "=========================================="
echo "   Moli Code Installation Script"
echo "=========================================="
echo ""
...
install_nvm() {
    ...
    if "${DOWNLOAD_CMD}" "${DOWNLOAD_ARGS}" "https://moli-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install_nvm.sh" > "${NVM_INSTALL_TEMP}"; then
    ...
    # Check if profile file is writable
        ...
        {
            echo ""
            echo "# NVM configuration (added by Moli Code installer)"
...
fix_npm_permissions() {
    ...
    if [[ "${use_user_dir}" == true ]]; then
        ...
        if [[ -n "${PROFILE_FILE}" ]] && ! grep -q '.npm-global/bin' "${PROFILE_FILE}" 2>/dev/null; then
            {
                echo ""
                echo "# NPM global bin (added by Moli Code installer)"
...
install_moli_code() {
    ...
    if command_exists moli-code; then
        local MOLI_VERSION
        MOLI_VERSION=$(moli-code --version 2>/dev/null || echo "unknown")
        log_success "Moli Code is already installed: ${MOLI_VERSION}"
    ...
    # Install Moli Code
    log_info "Installing Moli Code..."
    if npm install -g @dobby/moli-code@latest --registry https://registry.npmmirror.com; then
        log_success "Moli Code installed successfully!"

        # Verify installation
        if command_exists moli-code; then
            local moli_version
            moli_version=$(moli-code --version 2>/dev/null) || moli_version="unknown"
            log_info "Moli Code version: ${moli_version}"
        fi
    else
        log_error "Failed to install Moli Code!"
...
create_source_json() {
    ...
    cat > "${MOLI_DIR}/source.json" <<EOF
{
  "source": "${ESCAPED_SOURCE}"
}
EOF

    log_success "Installation source saved to ~/.moli/source.json"
}
...
main() {
    ...
    # Check if moli-code is immediately available
    if command_exists moli-code; then
        log_success "Moli Code is ready to use!"
        echo ""
        echo "You can now run: moli-code"
        echo ""
        # Auto-start moli-code
        log_info "Starting Moli Code..."
        echo ""
        exec moli-code
    else
        log_warning "Moli Code command not found in current session"
        echo ""
        echo "To use Moli Code immediately without restarting your terminal,"
        echo "run the following command in your current shell:"
        echo "  eval \$(${0} --print-env)"
        echo ""
        log_info "Or simply restart your terminal, then run: moli-code"
    fi
}
