...
echo ===========================================
echo Moli Code Installation Script with Source Tracking
echo ===========================================
echo.
...
if !ERRORLEVEL! EQU 0 (
    ...
    if !MAJOR_VERSION! GEQ 20 (
        ...
        if !ERRORLEVEL! NEQ 0 (
            echo ERROR: Failed to install Node.js. Cannot continue with Moli Code installation.
            exit /b 1
        )
    )
) else (
...
REM Verify npm is available before installing Moli Code
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ERROR: Failed to install Moli Code.
    exit /b 1
)
...
set "TEMP_DIR=%TEMP%\moli-nodejs-install"
