...
describe('Storage – getRuntimeBaseDir / setRuntimeBaseDir', () => {
  ...
  it('resolves relative paths in setRuntimeBaseDir using explicit cwd', () => {
    ...
    Storage.setRuntimeBaseDir('.moli', cwd);
    expect(Storage.getRuntimeBaseDir()).toBe(path.join(cwd, '.moli'));
  });
...
describe('Storage – config paths remain at ~/.moli regardless of runtime dir', () => {
  const originalEnv = process.env['QWEN_RUNTIME_DIR'];
...
describe('Storage – runtime base dir async context isolation', () => {
  ...
  it('uses contextual runtime dir inside runWithRuntimeBaseDir', async () => {
    ...
    await Storage.runWithRuntimeBaseDir('.moli', cwd, async () => {
      expect(Storage.getRuntimeBaseDir()).toBe(path.join(cwd, '.moli'));
    });
  });
  ...
  it('keeps concurrent contexts isolated', async () => {
    ...
    const runA = Storage.runWithRuntimeBaseDir('.moli-a', cwdA, async () => {
      await new Promise((resolve) => setTimeout(resolve, 10));
      return Storage.getRuntimeBaseDir();
    });

    const runB = Storage.runWithRuntimeBaseDir('.moli-b', cwdB, async () => {
      await new Promise((resolve) => setTimeout(resolve, 1));
      return Storage.getRuntimeBaseDir();
    });

    const [a, b] = await Promise.all([runA, runB]);
    expect(a).toBe(path.join(cwdA, '.moli-a'));
    expect(b).toBe(path.join(cwdB, '.moli-b'));
  });
});
