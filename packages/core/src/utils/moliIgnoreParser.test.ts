...
import { MoliIgnoreParser } from './moliIgnoreParser.js';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import * as os from 'node:os';

describe('MoliIgnoreParser', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'moliignore-test-'));
  });
  ...
  describe('when .moliignore exists', () => {
    beforeEach(async () => {
    ...
    it('should ignore files specified in .moliignore', () => {
      const parser = new MoliIgnoreParser(projectRoot);
  ...
  describe('when .moliignore does not exist', () => {
