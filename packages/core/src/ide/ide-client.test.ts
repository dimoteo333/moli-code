...
describe('IdeClient', () => {
  ...
  beforeEach(async () => {
    ...
    process.env['MOLI_CODE_IDE_WORKSPACE_PATH'] = '/test/workspace';
    delete process.env['MOLI_CODE_IDE_SERVER_PORT'];
    delete process.env['MOLI_CODE_IDE_SERVER_STDIO_COMMAND'];
    delete process.env['MOLI_CODE_IDE_SERVER_STDIO_ARGS'];
  ...
  describe('connect', () => {
    it('should connect using HTTP when port is provided in config file', async () => {
      process.env['MOLI_CODE_IDE_SERVER_PORT'] = '8080';
      ...
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/home/test', '.moli', 'ide', '8080.lock'),
        'utf8',
      );
    ...
    it('should connect using HTTP when port is provided in environment variables', async () => {
      ...
      process.env['MOLI_CODE_IDE_SERVER_PORT'] = '9090';
    ...
    it('should try a newer lock-file port when the configured port is stale', async () => {
      process.env['MOLI_CODE_IDE_SERVER_PORT'] = '1111';
      const primaryConfig = {
        port: '1111',
      ...
      vi.mocked(fs.promises.readFile).mockImplementation(
        async (filePath: fs.PathLike | FileHandle) => {
          const file = String(filePath);
          if (file === path.join('/home/test', '.moli', 'ide', '1111.lock')) {
            return JSON.stringify(primaryConfig);
          }
          if (file === path.join('/home/test', '.moli', 'ide', '2222.lock')) {
    ...
    it('should connect using stdio when stdio config is in environment variables', async () => {
      ...
      process.env['MOLI_CODE_IDE_SERVER_STDIO_COMMAND'] = 'env-cmd';
      process.env['MOLI_CODE_IDE_SERVER_STDIO_ARGS'] = '["--bar"]';
  ...
  describe('getConnectionConfigFromFile', () => {
    it('should return config from the env port lock file if it exists', async () => {
      process.env['MOLI_CODE_IDE_SERVER_PORT'] = '1234';
      ...
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/home/test', '.moli', 'ide', '1234.lock'),
        'utf8',
      );
      delete process.env['MOLI_CODE_IDE_SERVER_PORT'];
    });
    ...
    it('should read legacy pid config when available', async () => {
      ...
      expect(result).toEqual(config);
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp', 'moli-code-ide-server-12345.json'),
        'utf8',
      );
    });

    it('should fall back to legacy port file when pid file is missing', async () => {
      process.env['MOLI_CODE_IDE_SERVER_PORT'] = '2222';
      const config2 = { port: '2222', workspacePath: '/test/workspace2' };
      vi.mocked(fs.promises.readFile)
        .mockRejectedValueOnce(new Error('not found')) // ~/.moli/ide/<port>.lock
      ...
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp', 'moli-code-ide-server-2222.json'),
        'utf8',
      );
      delete process.env['MOLI_CODE_IDE_SERVER_PORT'];
    });
    ...
    it('should fall back to legacy config when env lock file has invalid JSON', async () => {
      process.env['MOLI_CODE_IDE_SERVER_PORT'] = '3333';
    ...
    it('should keep a live lock file even when it is older than 7 days', async () => {
      ...
      vi.mocked(fs.promises.readFile).mockImplementation(
        async (filePath: fs.PathLike | FileHandle) => {
          const file = String(filePath);
          if (file === path.join('/tmp', 'moli-code-ide-server-12345.json')) {
            throw new Error('not found');
          }
          if (file === path.join('/home/test', '.moli', 'ide', '1000.lock')) {
    ...
    it('should keep incomplete old lock files when there is no stronger stale signal', async () => {
      ...
      vi.mocked(fs.promises.readFile).mockImplementation(
        async (filePath: fs.PathLike | FileHandle) => {
          ...
          if (file === path.join('/home/test', '.moli', 'ide', '2000.lock')) {
