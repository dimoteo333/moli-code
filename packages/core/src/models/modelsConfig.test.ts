...
describe('ModelsConfig', () => {
  ...
  it('should apply Qwen OAuth apiKey placeholder during syncAfterAuthRefresh for fresh users', () => {
    ...
    // Config.refreshAuth passes modelId from modelsConfig.getModel(), which falls back to DEFAULT_MOLI_MODEL.
  ...
  it('should clear manual credentials when switching from USE_OPENAI to MOLI_OAUTH', () => {
  ...
  it('should maintain consistency between currentModelId and _generationConfig.model after initialization', () => {
    ...
    expect(config3.getModel()).toBe('coder-model'); // Falls back to DEFAULT_MOLI_MODEL
    ...
    it('should handle reload with empty config', async () => {
      ...
      expect(
        modelsConfig
          ...
          .filter((m) => m.authType !== 'moli-oauth').length,
      ).toBeGreaterThan(0);
