...
interface MockSharedTokenManager {
  getValidCredentials(moliClient: MoliOAuth2Client): Promise<MoliCredentials>;
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    async getValidCredentials(
      moliClient: MoliOAuth2Client,
    ): Promise<MoliCredentials> {
      // Try to get credentials from the client first
      const clientCredentials = moliClient.getCredentials();
  ...
  describe('isErrorResponse', () => {
    ...
    it('should return false for successful responses', () => {
      const successResponse: DeviceAuthorizationResponse = {
        device_code: 'test-device-code',
        user_code: 'TEST123',
        verification_uri: 'https://chat.moli.ai/device',
        verification_uri_complete: 'https://chat.moli.ai/device?code=TEST123',
        expires_in: 1800,
      };
...
describe('MoliOAuth2Client', () => {
  let client: MoliOAuth2Client;
  let originalFetch: typeof global.fetch;

  beforeEach(() => {
    // Create client instance
    client = new MoliOAuth2Client();
    ...
    it('should refresh token if access token is expired', async () => {
      ...
      (
        client as unknown as {
          sharedManager: {
            getValidCredentials: () => Promise<MoliCredentials>;
          };
        }
      ).sharedManager = {
...
describe('getMoliOAuthClient', () => {
  ...
  it('should load cached credentials if available', async () => {
    ...
    const client = await import('./moliOAuth2.js').then((module) =>
      module.getMoliOAuthClient(mockConfig),
    );
  ...
  it('should handle cached credentials refresh failure', async () => {
    ...
    await expect(
      import('./moliOAuth2.js').then((module) =>
        module.getMoliOAuthClient(mockConfig),
      ),
  ...
  it('should include troubleshooting hints when device auth fetch fails', async () => {
    ...
    const emitSpy = vi.spyOn(moliOAuth2Events, 'emit');

    let thrownError: unknown;
    try {
      const { getMoliOAuthClient } = await import('./moliOAuth2.js');
      await getMoliOAuthClient(mockConfig);
    } catch (error: unknown) {
      thrownError = error;
    }
...
describe('CredentialsClearRequiredError', () => {
  it('should create error with correct name and message', async () => {
    const { CredentialsClearRequiredError } = await import('./moliOAuth2.js');
...
describe('clearMoliCredentials', () => {
  it('should successfully clear credentials file', async () => {
    const { promises: fs } = await import('node:fs');
    const { clearMoliCredentials } = await import('./moliOAuth2.js');

    vi.mocked(fs.unlink).mockResolvedValue(undefined);

    await expect(clearMoliCredentials()).resolves.not.toThrow();
...
describe('MoliOAuth2Client - Additional Error Scenarios', () => {
  let client: MoliOAuth2Client;
  let originalFetch: typeof global.fetch;
...
describe('getMoliOAuthClient - Enhanced Error Scenarios', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
...
describe('authWithMoliDeviceFlow - Comprehensive Testing', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
...
describe('Event Emitter Integration', () => {
  it('should export moliOAuth2Events as EventEmitter', async () => {
    const { moliOAuth2Events } = await import('./moliOAuth2.js');
    expect(moliOAuth2Events).toBeInstanceOf(EventEmitter);
  });

  it('should define correct event enum values', async () => {
    const { MoliOAuth2Event } = await import('./moliOAuth2.js');
    expect(MoliOAuth2Event.AuthUri).toBe('auth-uri');
    expect(MoliOAuth2Event.AuthProgress).toBe('auth-progress');
    expect(MoliOAuth2Event.AuthCancel).toBe('auth-cancel');
  });
});
...
describe('Utility Functions', () => {
  ...
  describe('getMoliCachedCredentialPath', () => {
    it('should return correct path to cached credentials', async () => {
      const os = await import('os');
      const path = await import('path');

      const expectedPath = path.join(os.homedir(), '.moli', 'oauth_creds.json');

      // Since this is a private function, we test it indirectly through clearMoliCredentials
      const { promises: fs } = await import('node:fs');
      const { clearMoliCredentials } = await import('./moliOAuth2.js');

      vi.mocked(fs.unlink).mockResolvedValue(undefined);

      await clearMoliCredentials();
...
describe('Credential Caching Functions', () => {
  describe('cacheMoliCredentials', () => {
    it('should create directory and write credentials to file', async () => {
      // Mock the internal cacheMoliCredentials function by creating client and calling refresh
      const client = new MoliOAuth2Client();
      client.setCredentials({
        refresh_token: 'test-refresh',
      });
...
describe('Enhanced Error Handling and Edge Cases', () => {
  ...
  describe('MoliOAuth2Client getAccessToken enhanced scenarios', () => {
  ...
  describe('Enhanced refreshAccessToken scenarios', () => {
    it('should call clearMoliCredentials on 400 error', async () => {
      client.setCredentials({
...
describe('SharedTokenManager Integration in MoliOAuth2Client', () => {
  let client: MoliOAuth2Client;
  ...
  it('should handle TokenManagerError types correctly in getMoliOAuthClient', async () => {
    const mockConfig = {
    ...
    for (const errorInfo of tokenErrors) {
      ...
      try {
        await import('./moliOAuth2.js').then((module) =>
          module.getMoliOAuthClient(mockConfig),
        );
      } catch {
...
describe('Constants and Configuration', () => {
  it('should have correct OAuth endpoints', async () => {
    ...
    expect(url).toBe('https://chat.moli.ai/api/v1/oauth2/device/code');
  });
