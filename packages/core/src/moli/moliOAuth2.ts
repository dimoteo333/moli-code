...
// OAuth Endpoints
const MOLI_OAUTH_BASE_URL = 'https://chat.moli.ai';
 ...
 */
export interface MoliCredentials {
  access_token?: string;
 ...
 */
export interface IMoliOAuth2Client {
  setCredentials(credentials: MoliCredentials): void;
  getCredentials(): MoliCredentials;
 ...
 */
export class MoliOAuth2Client implements IMoliOAuth2Client {
  private credentials: MoliCredentials = {};
...
export async function getMoliOAuthClient(
  config: Config,
  options?: { requireCachedCredentials?: boolean },
): Promise<MoliOAuth2Client> {
  ...
  try {
    ...
    const result = await authWithMoliDeviceFlow(client, config);
    if (!result.success) {
...
async function authWithMoliDeviceFlow(
  client: MoliOAuth2Client,
  config: Config,
): Promise<AuthResult> {
  ...
  moliOAuth2Events.once(MoliOAuth2Event.AuthCancel, cancelHandler);
  ...
  const checkCancellation = (): AuthResult | null => {
    ...
    debugLogger.debug('\n' + message);
    moliOAuth2Events.emit(MoliOAuth2Event.AuthProgress, 'error', message);
  ...
  const emitAuthProgress = (
    ...
    moliOAuth2Events.emit(MoliOAuth2Event.AuthProgress, status, message);
  };
  ...
  try {
    ...
    moliOAuth2Events.emit(MoliOAuth2Event.AuthUri, deviceAuth);
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (isDeviceTokenSuccess(tokenResponse)) {
          ...
          // Convert to MoliCredentials format
          ...
          await cacheMoliCredentials(credentials);

          // IMPORTANT:
          // SharedTokenManager maintains an in-memory cache and throttles file checks.
          // If we only write the creds file here, a subsequent `getMoliOAuthClient()`
    ...
    moliOAuth2Events.off(MoliOAuth2Event.AuthCancel, cancelHandler);
  }
}

async function cacheMoliCredentials(credentials: MoliCredentials) {
...
export const clearCachedCredentialFile = clearMoliCredentials;
