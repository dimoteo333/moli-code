...
import { type MoliCredentials, type ErrorData } from './moliOAuth2.js';
import type {
...
import { MoliContentGenerator } from './moliContentGenerator.js';
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    private mockCredentials: MoliCredentials | null = null;
    ...
    async getValidCredentials(
      ...
      try {
        const { token } = await moliClient.getAccessToken();
        if (token) {
          const credentials = moliClient.getCredentials();
          return credentials;
        }
      } catch (error) {
        ...
        if (isAuthError) {
          ...
          try {
            const refreshResult = await moliClient.refreshAccessToken();
            if (refreshResult && !('error' in refreshResult)) {
              // Refresh succeeded, update client credentials and return them
              const updatedCredentials = moliClient.getCredentials();
              return updatedCredentials;
            } else {
              // Refresh failed, throw appropriate error
              throw new Error(
                'Failed to obtain valid Moli access token. Please re-authenticate.',
              );
            }
          } catch {
            throw new Error(
    ...
    setMockCredentials(credentials: MoliCredentials | null): void {
...
describe('MoliContentGenerator', () => {
  let mockMoliClient: IMoliOAuth2Client;
  let moliContentGenerator: MoliContentGenerator;
  let mockConfig: Config;

  const mockCredentials: MoliCredentials = {
  ...
  beforeEach(() => {
    ...
    // Create MoliContentGenerator instance
    ...
    moliContentGenerator = new MoliContentGenerator(
      mockMoliClient,
  ...
  describe('Core Content Generation Methods', () => {
    it('should generate content with valid token', async () => {
      vi.mocked(mockMoliClient.getAccessToken).mockResolvedValue({
        token: 'valid-token',
      });
      vi.mocked(mockMoliClient.getCredentials).mockReturnValue(mockCredentials);
      ...
      const result = await moliContentGenerator.generateContent(
        request,
        'test-prompt-id',
      );

      expect(result.text).toBe('Generated content');
      expect(mockMoliClient.getAccessToken).toHaveBeenCalled();
    });
    ...
    it('should generate content stream with valid token', async () => {
      ...
      const stream = await moliContentGenerator.generateContentStream(
        request,
        'test-prompt-id',
      );
      const chunks: string[] = [];
    ...
    it('should count tokens without requiring authentication', async () => {
      ...
      const result = await moliContentGenerator.countTokens(request);

      expect(result.totalTokens).toBe(15);
      // countTokens is a local operation and should not require OAuth credentials
      expect(mockMoliClient.getAccessToken).not.toHaveBeenCalled();
    });
    ...
    it('should embed content with valid token', async () => {
      ...
      const result = await moliContentGenerator.embedContent(request);
  ...
  describe('Token Management and Refresh Logic', () => {
    it('should refresh token on auth error and retry', async () => {
      ...
      vi.mocked(mockMoliClient.getAccessToken)
        .mockRejectedValueOnce(authError)
        .mockResolvedValueOnce({ token: 'refreshed-token' });

      // Refresh succeeds
      vi.mocked(mockMoliClient.refreshAccessToken).mockResolvedValue({
      ...
      vi.mocked(mockMoliClient.getCredentials).mockReturnValue({
        access_token: 'refreshed-token',
        token_type: 'Bearer',
      ...
      expect(result.text).toBe('Generated content');
      expect(mockMoliClient.refreshAccessToken).toHaveBeenCalled();
    });
    ...
    it('should handle token refresh failure', async () => {
      ...
      await expect(
        moliContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow(
        'Failed to obtain valid Moli access token. Please re-authenticate.',
      );
    ...
    it('should update endpoint when token is refreshed', async () => {
      ...
      await moliContentGenerator.generateContent(request, 'test-prompt-id');

      expect(mockMoliClient.getCredentials).toHaveBeenCalled();
    });
  });
  ...
  describe('Endpoint URL Normalization', () => {
    it('should use default endpoint when no custom endpoint provided', async () => {
      ...
      const parentPrototype = Object.getPrototypeOf(
        Object.getPrototypeOf(moliContentGenerator),
      );
  ...
  describe('Client State Management', () => {
    it('should set dynamic credentials during operations', async () => {
      const client = (
        moliContentGenerator as unknown as {
  ...
  describe('Error Handling and Retry Logic', () => {
    it('should retry once on authentication errors', async () => {
      ...
      vi.mocked(mockMoliClient.getAccessToken).mockImplementation(async () => {
    ...
    it('should not retry non-authentication errors', async () => {
      ...
      expect(mockMoliClient.refreshAccessToken).not.toHaveBeenCalled();
    ...
    it('should handle error response from token refresh', async () => {
      vi.mocked(mockMoliClient.getAccessToken).mockRejectedValue(
      ...
      await expect(
        moliContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow('Failed to obtain valid Moli access token');
    });
  });

  describe('Token State Management', () => {
    it('should cache and return current token', () => {
      expect(moliContentGenerator.getCurrentToken()).toBeNull();

      // Simulate setting a token internally
      (
        moliContentGenerator as unknown as { currentToken: string }
      ).currentToken = 'cached-token';

      expect(moliContentGenerator.getCurrentToken()).toBe('cached-token');
    });

    it('should clear token on clearToken()', () => {
      // Simulate having cached token value
      const moliInstance = moliContentGenerator as unknown as {
        currentToken: string;
      };
      moliInstance.currentToken = 'cached-token';

      moliContentGenerator.clearToken();
    ...
    it('should handle concurrent token refresh requests', async () => {
      ...
      vi.mocked(mockMoliClient.getAccessToken).mockRejectedValue(authError);
      vi.mocked(mockMoliClient.getCredentials).mockReturnValue(mockCredentials);

      vi.mocked(mockMoliClient.refreshAccessToken).mockImplementation(
        async () => {
          refreshCallCount++;
  ...
  describe('SharedTokenManager Integration', () => {
    it('should use SharedTokenManager to get valid credentials', async () => {
      ...
      const newGenerator = new MoliContentGenerator(
        mockMoliClient,
        { model: 'moli-turbo', authType: AuthType.MOLI_OAUTH },
        mockConfig,
      );
  ...
  describe('getCurrentEndpoint Method', () => {
    it('should handle URLs with custom ports', () => {
      ...
      endpoints.forEach(({ input, expected }) => {
        ...
        const generator = moliContentGenerator as unknown as {
  ...
  describe('Stream Error Handling', () => {
    it('should set credentials when stream generation fails', async () => {
      ...
      try {
        await moliContentGenerator.generateContentStream(
          request,
          'test-prompt-id',
        );
      } catch (error) {
  ...
  describe('Constructor and Initialization', () => {
    it('should initialize with configured base URL when provided', () => {
      const generator = new MoliContentGenerator(
        mockMoliClient,
        {
          model: 'moli-turbo',
          authType: AuthType.MOLI_OAUTH,
  ...
  describe('Edge Cases and Error Conditions', () => {
    ...
    it('should handle method types with token failure (except countTokens)', async () => {
      ...
      await expect(newGenerator.embedContent(embedRequest)).rejects.toThrow(
        'Failed to obtain valid Moli access token',
      );
