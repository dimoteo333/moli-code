  ...
  try {
    ...
    for (const file of mdFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".moli" in markdown files
  ...
  try {
    ...
    for (const file of scriptFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".moli" in shell scripts
        // Only match path references (e.g., ~/.claude/, $HOME/.claude, ./.claude/)
        // Avoid matching URLs, comments, or string literals containing .claude
        ...
        if (finalScriptContent !== content) {
          ...
          debugLogger.debug(
            `Updated transcript format and replaced .claude with .moli in shell script: ${filePath}`,
          );
        }
      } catch (error) {
        debugLogger.warn(
