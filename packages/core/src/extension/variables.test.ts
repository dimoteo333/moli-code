...
describe('substituteHookVariables', () => {
  ...
  it('should handle multiple event types with hooks', () => {
    const basePath = '/home/user/.moli/extensions/my-extension';
    ...
    expect(result!['PreToolUse']).toHaveLength(1);
    expect(result!['PreToolUse']![0].hooks![0].command).toBe(
      '/home/user/.moli/extensions/my-extension/scripts/pre-start.sh',
    );
    expect(result!['UserPromptSubmit']).toHaveLength(1);
    expect(result!['UserPromptSubmit']![0].hooks![0].command).toBe(
      '/home/user/.moli/extensions/my-extension/setup/install.py',
    );
  });
...
describe('performVariableReplacement', () => {
  ...
  it('should replace .claude with .moli in markdown files', () => {
    const extDir = path.join(testDir, 'ext');
    fs.mkdirSync(extDir, { recursive: true });
    ...
    expect(result).toContain('.moli/loop.local.md');
    expect(result).toContain('rm .moli/loop.local.md');
    expect(result).toContain('$HOME/.moli/cache');
  ...
  it('should replace .claude with .moli in shell scripts', () => {
    ...
    expect(result).toContain('./.moli/local');
    expect(result).toContain('.moli/config');
    // Note: URLs are also being replaced in current implementation
    expect(result).toContain('https://example.com/.moli/page');
  });
