...
export class Logger {
  private moliDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.moliDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.moliDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.moliDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.moliDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.moliDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.moliDir) {
      this.debugLogger.error(
