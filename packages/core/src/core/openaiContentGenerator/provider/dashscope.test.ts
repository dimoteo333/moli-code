...
describe('DashScopeOpenAICompatibleProvider', () => {
  ...
  describe('isDashScopeProvider', () => {
    it('should return true for MOLI_OAUTH auth type', () => {
      const config = {
        authType: AuthType.MOLI_OAUTH,
  ...
  describe('buildHeaders', () => {
    it('should build DashScope-specific headers', () => {
      ...
      expect(headers).toEqual({
        ...
        'X-DashScope-UserAgent': `MoliCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-AuthType': AuthType.MOLI_OAUTH,
      });
    });
    ...
    it('should merge custom headers with DashScope defaults', () => {
      ...
      expect(headers['User-Agent']).toContain('MoliCode/1.0.0');
      expect(headers['X-DashScope-UserAgent']).toContain('MoliCode/1.0.0');
      expect(headers['X-DashScope-AuthType']).toBe(AuthType.MOLI_OAUTH);
  ...
  describe('buildRequest', () => {
    ...
    it('should preserve all original request parameters', () => {
      ...
      expect(result.model).toBe('moli-max');
  ...
  describe('output token limits', () => {
    ...
    it('should limit max_tokens when it exceeds model limit for moli-vl-max-latest', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        model: 'moli-vl-max-latest',
        messages: [{ role: 'user', content: 'Hello' }],
    ...
    it('should set high resolution flag for moli-vl-plus', () => {
      const request: OpenAI.Chat.ChatCompletionCreateParams = {
        model: 'moli-vl-plus',
        messages: [
          {
