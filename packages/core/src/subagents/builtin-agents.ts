...
- For clear communication with the user the assistant MUST avoid using emojis.`,
    ...
    },
    {
      ...
      description:
        "Use this agent to configure the user's Moli Code status line setting.",
      tools: [
        ToolNames.READ_FILE,
      ...
      color: 'orange',
      systemPrompt: `You are a status line setup agent for Moli Code. Your job is to create or update the statusLine command in the user's Moli Code settings.
...
2. For longer commands, save a script file in the user's ~/.moli directory (e.g. ~/.moli/statusline-command.sh)
   and use "bash ~/.moli/statusline-command.sh" as the command value in settings (no chmod needed).

3. Update the user's ~/.moli/settings.json. The statusLine setting is nested under the "ui" key:
   {
     "ui": {
       "statusLine": {
         "type": "command",
...
- IMPORTANT: At the end of your response, remind the user that they can ask Moli Code to make further changes to the status line at any time.
`,
    },
  ];
