...
describe('SkillManager', () => {
  ...
  const validSkillConfig: SkillConfig = {
    ...
    description: 'A test skill',
    level: 'project',
    filePath: '/test/project/.moli/skills/test-skill/SKILL.md',
...
`;
    ...
    it('should determine level from file path', () => {
      const projectPath = '/test/project/.moli/skills/test-skill/SKILL.md';
      const userPath = '/home/user/.moli/skills/test-skill/SKILL.md';
...
`;
  ...
  describe('listSkills', () => {
    beforeEach(() => {
      ...
      const userQwenSkillsDir = path.join('/home/user', '.moli', 'skills');
...
Skill 3 content`);
    ...
    it('should deduplicate same-name skills across provider dirs within a level', async () => {
      // Override readdir to return the same skill name from both .moli and .agents dirs
      vi.mocked(fs.readdir).mockReset();
      const projectQwenDir = path.join('/test/project', '.moli', 'skills');
      ...
      vi.mocked(fs.readFile).mockImplementation((filePath) => {
        const pathStr = String(filePath);
        if (pathStr.includes('.moli') && pathStr.includes('shared-skill')) {
          return Promise.resolve(
      ...
      // Only one instance should remain, from .moli (first in PROVIDER_CONFIG_DIRS)
  ...
  describe('getSkillsBaseDirs', () => {
    it('should return all project-level base dirs', () => {
      ...
      expect(baseDirs).toContain(path.join('/test/project', '.moli', 'skills'));
    ...
    it('should return all user-level base dirs', () => {
      ...
      expect(baseDirs).toHaveLength(2);
      expect(baseDirs).toContain(path.join('/home/user', '.moli', 'skills'));
  ...
  describe('bundled skills', () => {
    ...
    const projectDirSegment = path.join('.moli', 'skills');
    const userDirSegment = path.join('.moli', 'skills');
    ...
    function mockReaddirForLevels(levels: Set<string>) {
      vi.mocked(fs.readdir).mockImplementation((dirPath) => {
        const pathStr = String(dirPath);
        const isBundled =
          pathStr.endsWith(bundledDirSegment) && !pathStr.includes('.moli');
        const isProject =
