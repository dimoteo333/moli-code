...
Copy the pre-built workflows from the [`examples/workflows`](./common-workflow) directory to your repository's `.github/workflows` directory. Note: the `qwen-dispatch.yml` workflow must also be copied, which triggers the workflows to run.
...
- <a name="__input_use_qwen_code_assist"></a><a href="#user-content-__input_use_qwen_code_assist"><code>use*qwen_code_assist</code></a>: *(Optional, default: `false`)\_ Whether to use Code Assist for Moli Code model access instead of the default Moli Code API key.
...
[QWEN.md]: https://github.com/QwenLM/moli-code-action/blob/main/docs/cli/configuration.md#context-files-hierarchical-instructional-context
