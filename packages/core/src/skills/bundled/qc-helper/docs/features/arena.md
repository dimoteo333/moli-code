...
1. **Worktree setup**: Moli Code creates isolated Git worktrees for each agent at `~/.qwen/arena/<session-id>/worktrees/<model-name>/`. Each worktree mirrors your current working directory state exactly — including staged changes, unstaged changes, and untracked files.
...
Agent Arena is one of several planned multi-agent modes in Moli Code. **Agent Team** and **Agent Swarm** are not yet implemented — the table below describes their intended design for reference.
...
- **Manual parallel sessions**: Run multiple Moli Code sessions yourself in separate terminals with [Git worktrees](https://git-scm.com/docs/git-worktree) for full manual control
