...
> Use `/loop` and the cron scheduling tools to run prompts repeatedly, poll for status, or set one-time reminders within a Moli Code session.

Scheduled tasks let Moli Code re-run a prompt automatically on an interval. Use them to poll a deployment, babysit a PR, check back on a long-running build, or remind yourself to do something later in the session.

Tasks are session-scoped: they live in the current Moli Code process and are gone when you exit. Nothing is written to disk.
...
The `/loop` [bundled skill](skills.md) is the quickest way to schedule a recurring prompt. Pass an optional interval and a prompt, and Moli Code sets up a cron job that fires in the background while the session stays open.

```text
/loop 5m check if the deployment finished and tell me what happened
```

Moli Code parses the interval, converts it to a cron expression, schedules the job, and confirms the cadence and job ID. It then immediately executes the prompt once — you don't have to wait for the first cron fire.
...
Each time the job fires, Moli Code runs `/review-pr 1234` as if you had typed it.
...
For one-shot reminders, describe what you want in natural language instead of using `/loop`. Moli Code schedules a single-fire task that deletes itself after running.
...
Moli Code pins the fire time to a specific minute and hour using a cron expression and confirms when it will fire.

## Manage scheduled tasks

Ask Moli Code in natural language to list or cancel tasks, or reference the underlying tools directly.
...
Under the hood, Moli Code uses these tools:
...
The scheduler checks every second for due tasks and enqueues them when the session is idle. A scheduled prompt fires between your turns, not while Moli Code is mid-response. If Moli Code is busy when a task comes due, the prompt waits until the current turn ends.

All times are interpreted in your local timezone. A cron expression like `0 9 * * *` means 9am wherever you're running Moli Code, not UTC.
...
- Tasks only fire while Moli Code is running and idle. Closing the terminal or letting the session exit cancels everything.
- No catch-up for missed fires. If a task's scheduled time passes while Moli Code is busy on a long-running request, it fires once when Moli Code becomes idle, not once per missed interval.
- No persistence across restarts. Restarting Moli Code clears all session-scoped tasks.
