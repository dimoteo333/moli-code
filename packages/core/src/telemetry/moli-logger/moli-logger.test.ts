...
import { MoliLogger, TEST_ONLY } from './moli-logger.js';
...
describe('MoliLogger', () => {
  ...
  beforeEach(() => {
    ...
    (MoliLogger as any).instance = undefined;
  });
  ...
  describe('getInstance', () => {
    it('returns undefined when usage statistics are disabled', () => {
      ...
      const logger = MoliLogger.getInstance(config);
      expect(logger).toBeUndefined();
    });

    it('returns an instance when usage statistics are enabled', () => {
      const logger = MoliLogger.getInstance(mockConfig);
      expect(logger).toBeInstanceOf(MoliLogger);
    });

    it('is a singleton', () => {
      const logger1 = MoliLogger.getInstance(mockConfig);
      const logger2 = MoliLogger.getInstance(mockConfig);
