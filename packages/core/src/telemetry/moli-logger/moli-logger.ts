...
export class MoliLogger {
  private static instance: MoliLogger;
  ...
  static getInstance(config?: Config): MoliLogger | undefined {
    if (config === undefined || !config?.getUsageStatisticsEnabled())
      return undefined;
    if (!MoliLogger.instance) {
      MoliLogger.instance = new MoliLogger(config);
    }

    return MoliLogger.instance;
  }
    ...
    try {
      ...
      if (wasAtCapacity) {
        this.debugLogger.debug(
          `MoliLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`,
        );
      }
    } catch (error) {
      this.debugLogger.error('MoliLogger: Failed to enqueue log event.', error);
    }
  }
  ...
  readSourceInfo(): string {
    try {
      const sourceJsonPath = path.join(os.homedir(), '.moli', 'source.json');
  ...
  async flushToRum(): Promise<LogResponse> {
    if (this.isFlushInProgress) {
      this.debugLogger.debug(
        'MoliLogger: Flush already in progress, marking pending flush.',
      );
  ...
  private requeueFailedEvents(eventsToSend: RumEvent[]): void {
    ...
    this.debugLogger.debug(
      `MoliLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`,
    );
  }
}
