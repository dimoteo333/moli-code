...
export async function resolveTelemetrySettings(options: {
  ...
  const enabled =
    argv.telemetry ??
    parseBooleanEnvFlag(env['MOLI_TELEMETRY_ENABLED']) ??
    settings.enabled;

  const rawTarget =
    (argv.telemetryTarget as string | TelemetryTarget | undefined) ??
    env['MOLI_TELEMETRY_TARGET'] ??
  ...
  const otlpEndpoint =
    ...
    env['MOLI_TELEMETRY_OTLP_ENDPOINT'] ??
    env['OTEL_EXPORTER_OTLP_ENDPOINT'] ??
    settings.otlpEndpoint;

  const rawProtocol =
    (argv.telemetryOtlpProtocol as string | undefined) ??
    env['MOLI_TELEMETRY_OTLP_PROTOCOL'] ??
  ...
  const logPrompts =
    ...
    parseBooleanEnvFlag(env['MOLI_TELEMETRY_LOG_PROMPTS']) ??
    settings.logPrompts;

  const outfile =
    argv.telemetryOutfile ?? env['MOLI_TELEMETRY_OUTFILE'] ?? settings.outfile;

  const useCollector =
    parseBooleanEnvFlag(env['MOLI_TELEMETRY_USE_COLLECTOR']) ??
