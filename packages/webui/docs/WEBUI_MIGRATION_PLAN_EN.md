...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@moli-code/webui` package
...
```typescript
// @moli-code/webui/src/context/PlatformContext.ts
...
```javascript
// @moli-code/webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
```
Developer ──► @moli-code/webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@moli-code/webui';
// or import { FileIcon } from '@moli-code/webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@moli-code/webui`
...
│     └── Edit files in @moli-code/webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@moli-code/webui'            │
...
1. **Single Source of Truth**: All UI components live in `@moli-code/webui`
