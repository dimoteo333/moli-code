...
import * as ServerConfig from '@dobby/moli-code-core';
...
describe('loadCliConfig', () => {
  ...
  it('should reset context file names to MOLI.md and AGENTS.md by default', async () => {
  ...
  it('should reset context filenames to defaults when context.fileName is not configured', async () => {
    ...
    const argv = await parseArguments();
    const settings: Settings = {};
    const defaultContextFiles = ['MOLI.md', 'AGENTS.md'];
    const getAllSpy = vi
...
describe('loadCliConfig model selection', () => {
  it.skip('selects a model from settings.json if provided', async () => {
    ...
    expect(config.getModel()).toBe('moli3-coder-plus');
  });
  ...
  it.skip('uses the default gemini model if nothing is set', async () => {
    ...
    expect(config.getModel()).toBe(DEFAULT_MOLI_MODEL);
  });
...
describe('Telemetry configuration via environment variables', () => {
  it('should prioritize MOLI_TELEMETRY_ENABLED over settings', async () => {
    vi.stubEnv('MOLI_TELEMETRY_ENABLED', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = { telemetry: { enabled: false } };
    const config = await loadCliConfig(settings, argv, undefined, []);
  ...
  it('should prioritize MOLI_TELEMETRY_TARGET over settings', async () => {
    vi.stubEnv('MOLI_TELEMETRY_TARGET', 'gcp');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = {
  ...
  it('should throw when MOLI_TELEMETRY_TARGET is invalid', async () => {
    vi.stubEnv('MOLI_TELEMETRY_TARGET', 'bogus');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = {
  ...
  it('should prioritize MOLI_TELEMETRY_OTLP_ENDPOINT over settings and default env var', async () => {
    vi.stubEnv('OTEL_EXPORTER_OTLP_ENDPOINT', 'http://default.env.com');
    vi.stubEnv('MOLI_TELEMETRY_OTLP_ENDPOINT', 'http://gemini.env.com');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = {
  ...
  it('should prioritize MOLI_TELEMETRY_OTLP_PROTOCOL over settings', async () => {
    vi.stubEnv('MOLI_TELEMETRY_OTLP_PROTOCOL', 'http');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
  ...
  it('should prioritize MOLI_TELEMETRY_LOG_PROMPTS over settings', async () => {
    vi.stubEnv('MOLI_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = { telemetry: { logPrompts: true } };
    const config = await loadCliConfig(settings, argv, undefined, []);
  ...
  it('should prioritize MOLI_TELEMETRY_OUTFILE over settings', async () => {
    vi.stubEnv('MOLI_TELEMETRY_OUTFILE', '/gemini/env/telemetry.log');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = {
  ...
  it('should prioritize MOLI_TELEMETRY_USE_COLLECTOR over settings', async () => {
    vi.stubEnv('MOLI_TELEMETRY_USE_COLLECTOR', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
  ...
  it('should use settings value when MOLI_TELEMETRY_ENABLED is not set', async () => {
    vi.stubEnv('MOLI_TELEMETRY_ENABLED', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = { telemetry: { enabled: true } };
    const config = await loadCliConfig(settings, argv, undefined, []);
  ...
  it('should use settings value when MOLI_TELEMETRY_TARGET is not set', async () => {
    vi.stubEnv('MOLI_TELEMETRY_TARGET', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = {
      telemetry: { target: 'local' },
  ...
  it("should treat MOLI_TELEMETRY_ENABLED='1' as true", async () => {
    vi.stubEnv('MOLI_TELEMETRY_ENABLED', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const config = await loadCliConfig({}, argv, undefined, []);
    expect(config.getTelemetryEnabled()).toBe(true);
  ...
  it("should treat MOLI_TELEMETRY_ENABLED='0' as false", async () => {
    vi.stubEnv('MOLI_TELEMETRY_ENABLED', '0');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const config = await loadCliConfig(
  ...
  it("should treat MOLI_TELEMETRY_LOG_PROMPTS='1' as true", async () => {
    vi.stubEnv('MOLI_TELEMETRY_LOG_PROMPTS', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const config = await loadCliConfig({}, argv, undefined, []);
    expect(config.getTelemetryLogPromptsEnabled()).toBe(true);
  ...
  it("should treat MOLI_TELEMETRY_LOG_PROMPTS='false' as false", async () => {
    vi.stubEnv('MOLI_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const config = await loadCliConfig(
