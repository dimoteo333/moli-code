...
describe('computeWindowTitle', () => {
  ...
  it('should use default Moli title when CLI_TITLE is not set', () => {
    const result = computeWindowTitle('my-project');
    expect(result).toBe('Moli - my-project');
  });
  ...
  it('should handle folder names with special characters', () => {
    ...
    expect(result).toBe('Moli - project-name_v1.0');
  });
});
