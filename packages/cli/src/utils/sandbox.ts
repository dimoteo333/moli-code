...
import type { Config, SandboxConfig } from '@dobby/moli-code-core';
import { FatalSandboxError } from '@dobby/moli-code-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'moli-code-sandbox';
const SANDBOX_NETWORK_NAME = 'moli-code-sandbox';
const SANDBOX_PROXY_NAME = 'moli-code-sandbox-proxy';
...
export async function start_sandbox(
  ...
  // determine full path for moli-code to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under moli-code repo
  //
  // note this can only be done with binary linked from moli-code repo
  if (process.env['BUILD_SANDBOX']) {
    if (!gcPath.includes('moli-code/packages/')) {
      throw new FatalSandboxError(
        'Cannot build sandbox using installed Moli Code binary; ' +
          'run `npm link ./packages/cli` under MoliCode-cli repo to switch to linked binary.',
      );
    } else {
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the moli-code repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or notify moli-code-dev@service.alibaba.com if the issue persists.';
    throw new FatalSandboxError(
  ...
  // tests that need to access host's ~/.moli (e.g., --resume functionality)
  ...
  if (useCurrentUser) {
    ...
    // allowing tests that need to access host's ~/.moli (e.g., --resume) to work.
