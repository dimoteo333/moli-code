...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/dimoteo333/moli-code-action/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest moli-code-action release:`,
      _error,
    );
    throw new Error(
      `Unable to determine the latest moli-code-action release on GitHub.`,
    );
  }
};
