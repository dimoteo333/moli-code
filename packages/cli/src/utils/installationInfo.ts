...
import { createDebugLogger, isGitRepository } from '@dobby/moli-code-core';
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        ...
        childProcess.execSync('brew list -1 | grep -q "^moli-code$"', {
          stdio: 'ignore',
        });
        return {
    ...
    if (realPath.includes('/.pnpm/global')) {
      const updateCommand = 'pnpm add -g @dobby/moli-code@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @dobby/moli-code@latest';
      return {
    ...
    if (realPath.includes('/.bun/bin')) {
      const updateCommand = 'bun add -g @dobby/moli-code@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @dobby/moli-code@latest';
    return {
