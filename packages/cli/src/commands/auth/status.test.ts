...
describe('showAuthStatus', () => {
  ...
  it('should show message when no authentication is configured', async () => {
    ...
    expect(writeStdoutLine).toHaveBeenCalledWith(
      expect.stringContaining('moli auth moli-oauth'),
    );
    expect(writeStdoutLine).toHaveBeenCalledWith(
      expect.stringContaining('moli auth coding-plan'),
    );
    expect(process.exit).toHaveBeenCalledWith(0);
  });

  it('should show Moli OAuth status when configured', async () => {
    vi.mocked(loadSettings).mockReturnValue(
      createMockSettings({
        security: {
          auth: {
            selectedType: AuthType.MOLI_OAUTH,
          },
        },
      }),
    );
    ...
    expect(writeStdoutLine).toHaveBeenCalledWith(
      expect.stringContaining('Moli OAuth'),
    );
    expect(writeStdoutLine).toHaveBeenCalledWith(
