...
interface MoliOAuthProgressProps {
  onTimeout: () => void;
  onCancel: () => void;
...
const debugLogger = createDebugLogger('MOLI_OAUTH_PROGRESS');
...
export function MoliOAuthProgress({
  onTimeout,
  onCancel,
  deviceAuth,
  authStatus,
  authMessage,
}: MoliOAuthProgressProps): React.JSX.Element {
  ...
  if (authStatus === 'error') {
    return (
      ...
      >
        <Text bold color={Colors.AccentRed}>
          Moli OAuth Authentication Error
        </Text>
    ...
    return (
      ...
      >
        <Box>
          <Text>
            <Spinner type="dots" />
            {t('Waiting for Moli OAuth authentication...')}
          </Text>
        </Box>
        <Box marginTop={1} justifyContent="space-between">
          <Text color={Colors.Gray}>
