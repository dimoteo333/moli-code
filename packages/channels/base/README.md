# @dobby/moli-code-channel-base

Base infrastructure for building Moli Code channel adapters. Provides the abstract base class, access control, session routing, and the ACP bridge that communicates with the agent.
...
```bash
npm install @dobby/moli-code-channel-base
```
...
```typescript
import { ChannelBase } from '@dobby/moli-code-channel-base';
import type {
  ChannelConfig,
  Envelope,
  AcpBridge,
} from '@dobby/moli-code-channel-base';
...
For a complete working example, see [`@moli-code/channel-plugin-example`](../plugin-example/).
...
| `AcpBridge`     | Spawns and communicates with the `moli-code --acp` agent process |
...
Manages the `moli-code --acp` child process and ACP sessions.
...
- [`@moli-code/channel-plugin-example`](../plugin-example/) — working reference implementation
