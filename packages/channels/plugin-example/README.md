# @moli-code/channel-plugin-example

A reference channel plugin for Moli Code. It connects to a WebSocket server and routes messages through the full channel pipeline (access control, session routing, agent bridge).
...
```bash
npm install @moli-code/channel-plugin-example
```

### 2. Link it as a Moli Code extension

The package ships a `qwen-extension.json` manifest, so it works as an extension out of the box:

```bash
qwen extensions link ./node_modules/@moli-code/channel-plugin-example
```
