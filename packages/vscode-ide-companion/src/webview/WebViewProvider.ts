        ...
        return new Promise((resolve) => {
          ...
          this.pendingPermissionResolve = (optionId: string) => {
            try {
              ...
              // If allowed/proceeded, close any open moli-diff editors and suppress re-open briefly
              const isCancel =
                optionId === 'cancel' ||
          ...
          const handler = (message: PermissionResponseMessage) => {
            ...
            if (isCancel) {
              // Close any open moli-diff editors first
              try {
                void vscode.commands.executeCommand('moli.diff.closeAll');
              } catch (err) {
                console.warn(
            ...
            // If user allowed/proceeded, proactively close any open moli-diff editors and suppress re-open briefly
            else {
              try {
                void vscode.commands.executeCommand('moli.diff.closeAll');
              } catch (err) {
