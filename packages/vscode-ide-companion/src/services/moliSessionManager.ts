 ...
 */
export class MoliSessionManager {
  ...
  private getSessionDir(workingDir: string): string {
    ...
    const sessionDir = path.join(this.moliDir, 'tmp', projectHash, 'chats');
    return sessionDir;
  }
  ...
  async saveSession(
    ...
    try {
      ...
      console.log(`[MoliSessionManager] Session saved: ${filePath}`);
      return sessionId;
    } catch (error) {
      console.error('[MoliSessionManager] Failed to save session:', error);
      throw error;
    }
  }
  ...
  async loadSession(
    ...
    try {
      ...
      if (!fs.existsSync(filePath)) {
        console.log(`[MoliSessionManager] Session file not found: ${filePath}`);
        return null;
      }

      const content = fs.readFileSync(filePath, 'utf-8');
      const session = JSON.parse(content) as MoliSession;

      console.log(`[MoliSessionManager] Session loaded: ${filePath}`);
      return session;
    } catch (error) {
      console.error('[MoliSessionManager] Failed to load session:', error);
      return null;
    }
  }
   ...
   */
  async listSessions(workingDir: string): Promise<MoliSession[]> {
    try {
      const sessionDir = this.getSessionDir(workingDir);
      ...
      for (const file of files) {
        try {
          ...
          console.error(
            `[MoliSessionManager] Failed to read session file ${file}:`,
            error,
          );
        }
      }
      ...
      console.error('[MoliSessionManager] Failed to list sessions:', error);
      return [];
    }
  }
  ...
  async deleteSession(sessionId: string, workingDir: string): Promise<boolean> {
    try {
      ...
      if (fs.existsSync(filePath)) {
        ...
        console.log(`[MoliSessionManager] Session deleted: ${filePath}`);
        return true;
      }

      return false;
    } catch (error) {
      console.error('[MoliSessionManager] Failed to delete session:', error);
      return false;
    }
  }
}
