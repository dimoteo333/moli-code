...
export class MoliSessionReader {
  ...
  async getAllSessions(
    ...
    try {
      ...
      if (!allProjects && workingDir) {
        ...
        const chatsDir = path.join(this.moliDir, 'tmp', projectHash, 'chats');
        const projectSessions = await this.readSessionsFromDir(chatsDir);
        sessions.push(...projectSessions);
      } else {
        // All projects
        const tmpDir = path.join(this.moliDir, 'tmp');
        if (!fs.existsSync(tmpDir)) {
          console.log('[MoliSessionReader] Tmp directory not found:', tmpDir);
          return [];
        }
      ...
      console.error('[MoliSessionReader] Failed to get sessions:', error);
      return [];
    }
  }
   ...
   */
  private async readSessionsFromDir(chatsDir: string): Promise<MoliSession[]> {
    const sessions: MoliSession[] = [];
    ...
    for (const file of jsonSessionFiles) {
      ...
      try {
        ...
        console.error(
          '[MoliSessionReader] Failed to read session file:',
          filePath,
          error,
        );
      }
    ...
    for (const file of jsonlSessionFiles) {
      ...
      try {
        ...
        console.error(
          '[MoliSessionReader] Failed to read JSONL session file:',
          filePath,
          error,
        );
      }
   ...
   */
  getSessionTitle(session: MoliSession): string {
  ...
  private async readJsonlSession(
    ...
    try {
      ...
      console.error(
        '[MoliSessionReader] Failed to parse JSONL session:',
        error,
      );
      return null;
    }
  ...
  async deleteSession(
    ...
    try {
      ...
      console.error('[MoliSessionReader] Failed to delete session:', error);
      return false;
    }
  }
}
