...
import {
  MoliConnectionHandler,
  type MoliConnectionResult,
} from '../services/moliConnectionHandler.js';
import { MoliSessionUpdateHandler } from './moliSessionUpdateHandler.js';
...
export class MoliAgentManager {
  private connection: AcpConnection;
  private sessionReader: MoliSessionReader;
  private sessionManager: MoliSessionManager;
  private connectionHandler: MoliConnectionHandler;
  private sessionUpdateHandler: MoliSessionUpdateHandler;
  ...
  // Callback storage
  private callbacks: MoliAgentCallbacks = {};

  constructor() {
    this.connection = new AcpConnection();
    this.sessionReader = new MoliSessionReader();
    this.sessionManager = new MoliSessionManager();
    this.connectionHandler = new MoliConnectionHandler();
      ...
      try {
        ...
        if (
          ...
          if (update?.sessionUpdate === 'user_message_chunk' && text) {
            console.log(
              '[MoliAgentManager] Rehydration: routing user message chunk',
            );
          ...
          if (update?.sessionUpdate === 'agent_message_chunk' && text) {
            console.log(
              '[MoliAgentManager] Rehydration: routing agent message chunk',
            );
            this.callbacks.onMessage?.({
              role: 'assistant',
              content: text,
          ...
          console.log(
            '[MoliAgentManager] Rehydration: non-text update, forwarding to handler',
          );
        }
      } catch (err) {
        console.warn('[MoliAgentManager] Rehydration routing failed:', err);
      }
    ...
    this.connection.onEndTurn = (reason?: string) => {
      try {
        ...
        console.warn('[MoliAgentManager] onEndTurn callback error:', err);
      }
    };
    ...
    this.connection.onAuthenticateUpdate = (
      ...
      try {
        ...
        console.warn(
          '[MoliAgentManager] onAuthenticateUpdate callback error:',
          err,
        );
      }
    };
    ...
    this.connection.onInitialized = (init: unknown) => {
      try {
        ...
        console.warn('[MoliAgentManager] onInitialized parse error:', err);
      }
    };
  }
  ...
  async connect(
    ...
    if (res.availableModels && res.availableModels.length > 0) {
      console.log(
        '[MoliAgentManager] Emitting availableModels from connect():',
  ...
  async setApprovalModeFromUi(
    ...
    try {
      ...
      console.error('[MoliAgentManager] Failed to set mode:', err);
      throw err;
    }
  }
  ...
  async setModelFromUi(modelId: string): Promise<ModelInfo | null> {
    try {
      ...
      console.error('[MoliAgentManager] Failed to set model:', err);
      throw err;
    }
  }
  ...
  async validateCurrentSession(): Promise<boolean> {
    try {
      ...
      console.warn('[MoliAgentManager] Session validation failed:', error);
  ...
  async getSessionList(): Promise<Array<Record<string, unknown>>> {
    console.log(
      '[MoliAgentManager] Getting session list with version-aware strategy',
    );

    try {
      console.log(
        '[MoliAgentManager] Attempting to get session list via ACP method',
      );
      const response = await this.connection.listSessions();
      console.log('[MoliAgentManager] ACP session list response:', response);
      ...
      console.log(
        '[MoliAgentManager] Sessions retrieved via ACP:',
        res,
        items.length,
      );
      if (items.length > 0) {
      ...
      console.warn(
        '[MoliAgentManager] ACP session list failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log('[MoliAgentManager] Getting session list from file system');
      const sessions = await this.sessionReader.getAllSessions(undefined, true);
      console.log(
        '[MoliAgentManager] Session list from file system (all projects):',
        sessions.length,
      );

      const result = sessions.map(
        (session: MoliSession): Record<string, unknown> => ({
      ...
      console.log(
        '[MoliAgentManager] Sessions retrieved from file system:',
        result.length,
      );
      return result;
    } catch (error) {
      console.error(
        '[MoliAgentManager] Failed to get session list from file system:',
        error,
      );
      return [];
    }
  ...
  async getSessionListPaged(params?: {
    ...
    try {
      ...
      console.warn('[MoliAgentManager] Paged ACP session list failed:', error);
    ...
    try {
      ...
      console.error('[MoliAgentManager] File system paged list failed:', error);
  ...
  async getSessionMessages(sessionId: string): Promise<ChatMessage[]> {
    try {
      try {
        ...
        console.log(
          '[MoliAgentManager] Session list item for filePath lookup:',
          item,
        );
        if (
          typeof item === 'object' &&
        ...
        console.warn('[MoliAgentManager] JSONL read path lookup failed:', e);
      }
      ...
      console.error(
        '[MoliAgentManager] Failed to get session messages:',
        error,
      );
      return [];
    }
  ...
  private async readJsonlMessages(filePath: string): Promise<ChatMessage[]> {
    ...
    try {
      ...
      console.log(
        '[MoliAgentManager] JSONL records read:',
        records.length,
        filePath,
      );
      ...
      console.log(
        '[MoliAgentManager] JSONL messages reconstructed:',
        msgs.length,
      );
      return msgs;
    } catch (err) {
      console.warn('[MoliAgentManager] Failed to read JSONL messages:', err);
      return [];
    }
  }
  ...
  async saveSessionViaCommand(
    ...
    try {
      console.log(
        '[MoliAgentManager] Saving session via /chat save command:',
        sessionId,
        'with tag:',
        tag,
      );
      ...
      console.log('[MoliAgentManager] /chat save command sent successfully');
      return {
        success: true,
        message: `Session saved with tag: ${tag}`,
      };
    } catch (error) {
      console.error('[MoliAgentManager] /chat save command failed:', error);
      return {
        success: false,
  ...
  async saveSessionViaAcp(
    ...
    console.warn(
      '[MoliAgentManager] saveSessionViaAcp is deprecated, using command-based save instead',
    );
  ...
  async loadSessionViaAcp(
    ...
    try {
      ...
      console.log(
        '[MoliAgentManager] Rehydration start for session:',
        sessionId,
      );
      console.log(
        '[MoliAgentManager] Attempting session/load via ACP for session:',
        sessionId,
      );
      ...
      console.log(
        '[MoliAgentManager] Session load succeeded. Response:',
      ...
      console.error(
        '[MoliAgentManager] Session load via ACP failed for session:',
        sessionId,
      );
      console.error('[MoliAgentManager] Error type:', error?.constructor?.name);
      console.error('[MoliAgentManager] Error message:', errorMessage);
      ...
      if (error && typeof error === 'object') {
        ...
        if ('error' in error) {
          ...
          if (acpError.error) {
            console.error(
              '[MoliAgentManager] ACP error code:',
              acpError.error.code,
            );
            console.error(
              '[MoliAgentManager] ACP error message:',
              acpError.error.message,
            );
          }
        } else {
          console.error('[MoliAgentManager] Non-ACPIf error details:', error);
        }
      }
      ...
      console.log('[MoliAgentManager] Rehydration end for session:', sessionId);
  ...
  async loadSession(sessionId: string): Promise<ChatMessage[] | null> {
    console.log(
      '[MoliAgentManager] Loading session with version-aware strategy:',
      sessionId,
    );

    try {
      console.log(
        '[MoliAgentManager] Attempting to load session via ACP method',
      );
      await this.loadSessionViaAcp(sessionId);
      console.log('[MoliAgentManager] Session loaded successfully via ACP');
      ...
      console.warn(
        '[MoliAgentManager] ACP session load failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log(
        '[MoliAgentManager] Loading session messages from file system',
      );
      const messages = await this.loadSessionMessagesFromFile(sessionId);
      console.log(
        '[MoliAgentManager] Session messages loaded successfully from file system',
      );
      return messages;
    } catch (error) {
      console.error(
        '[MoliAgentManager] Failed to load session messages from file system:',
        error,
      );
      return null;
    }
  ...
  private async loadSessionMessagesFromFile(
    ...
    try {
      console.log(
        '[MoliAgentManager] Loading session from file system:',
        sessionId,
      );
      ...
      if (!session) {
        console.log(
          '[MoliAgentManager] Session not found in file system:',
          sessionId,
        );
        return null;
      }
      ...
      console.error(
        '[MoliAgentManager] Session load from file system failed:',
        error,
      );
      throw error;
    }
  ...
  async createNewSession(
    ...
    if (this.connection.currentSessionId) {
      console.log(
        '[MoliAgentManager] createNewSession: reusing existing session',
    ...
    if (this.sessionCreateInFlight) {
      console.log(
        '[MoliAgentManager] createNewSession: session creation already in flight',
      );
      return this.sessionCreateInFlight;
    }

    console.log('[MoliAgentManager] Creating new session...');
    ...
    this.sessionCreateInFlight = (async () => {
      try {
        ...
        try {
          ...
          console.log(
            '[MoliAgentManager] newSession returned:',
          ...
          if (requiresAuth) {
            if (!autoAuthenticate) {
              console.warn(
                '[MoliAgentManager] session/new requires authentication but auto-auth is disabled. Deferring until user logs in.',
              );
              throw err;
            }
            console.warn(
              '[MoliAgentManager] session/new requires authentication. Retrying with authenticate...',
            );
            try {
              // Let CLI handle authentication - it's the single source of truth
              await this.connection.authenticate(authMethod);
              console.log(
                '[MoliAgentManager] createNewSession Authentication successful. Retrying session/new...',
              );
              ...
              console.error(
                '[MoliAgentManager] Re-authentication failed:',
                reauthErr,
              );
              throw reauthErr;
            }
        ...
        console.log(
          '[MoliAgentManager] Extracted model state from session/new:',
          modelState,
        );
        if (
          ...
          console.log(
            '[MoliAgentManager] Emitting availableModels:',
            modelState.availableModels,
          );
          if (this.callbacks.onAvailableModels) {
          ...
          console.warn(
            '[MoliAgentManager] No availableModels found in session/new response. Raw models field:',
        ...
        console.log(
          '[MoliAgentManager] New session created with ID:',
          newSessionId,
        );
        return newSessionId;
      } finally {
  ...
  async cancelCurrentPrompt(): Promise<void> {
    console.log('[MoliAgentManager] Cancelling current prompt');
