# 몰리 코드 PowerPoint 추가 기능 제거 스크립트
#
...
Write-Host '=== 몰리 코드 PowerPoint 추가 기능 제거 ===' -ForegroundColor Cyan
...
# 이 설치가 만든 TrustedCatalogs 항목 제거 (Url이 MoliPowerPointAddin 공유를 가리키는 것)
...
if (Test-Path $catRoot) {
    Get-ChildItem $catRoot | ForEach-Object {
        ...
        if ($url -like '*\MoliPowerPointAddin') {
            Remove-Item -Path $_.PSPath -Recurse -Force
        }
    }
}
Get-SmbShare -Name 'MoliPowerPointAddin' -ErrorAction SilentlyContinue |
...
Write-Host '참고: PowerPoint가 추가 기능을 캐시하는 경우 아래 폴더를 비우면 완전히 사라집니다:'
