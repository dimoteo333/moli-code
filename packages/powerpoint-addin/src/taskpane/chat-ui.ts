 ...
 * input row with local text-file attachments referenced as @filename.
 */
...
export interface ChatUiCallbacks {
  onSend: (text: string, attachments: LocalFileAttachment[]) => void;
  onStop: () => void;
}
...
export function createChatUi(
  ...
  // Attachment pill row (hidden until a local file is attached).
  ...
  attachBtn.title = STRINGS.attachFile;
  attachBtn.setAttribute('aria-label', STRINGS.attachFile);
  const fileInput = el('input', 'mc-file-input', inputRow) as HTMLInputElement;
  fileInput.type = 'file';
  ...
  // File snapshots referenced by inline @filename tokens in the prompt.
  ...
  function makeRemoveHandler(name: string): () => void {
    return function () {
  ...
  function renderAttachments(): void {
    ...
    for (let i = 0; i < attachments.length; i++) {
      ...
      el('span', 'mc-attach-pill-text', pill).textContent =
        '📌 ' + attachmentReference(att.name);
      const remove = el(
        'button',
        'mc-attach-remove',
        pill,
      ...
      remove.onclick = makeRemoveHandler(att.name);
    }
  ...
  function removeAttachment(name: string): void {
    ...
    const token = attachmentReference(name);
  ...
  function addAttachment(attachment: LocalFileAttachment): void {
    for (let i = 0; i < attachments.length; i++) {
      if (sameFileName(attachments[i].name, attachment.name)) {
        const previousName = attachments[i].name;
        attachments[i] = attachment; // refresh same-named snapshot
        if (previousName !== attachment.name) {
          const previousToken = attachmentReference(previousName);
          while (input.value.indexOf(previousToken) >= 0) {
            input.value = input.value.replace(
              previousToken,
              attachmentReference(attachment.name),
            );
          }
        }
        if (input.value.indexOf(attachmentReference(attachment.name)) < 0) {
          const prefix = input.value && !/\s$/.test(input.value) ? ' ' : '';
          insertAtCursor(prefix + attachmentReference(attachment.name) + ' ');
        }
        renderAttachments();
        return;
      }
  ...
  function showFileError(err: unknown): void {
    ...
    if (code === 'FILE_TOO_LARGE') {
      api.addSystemNote(STRINGS.fileTooLarge);
    } else if (code === 'UNSUPPORTED_TYPE' || code === 'BINARY_FILE') {
      api.addSystemNote(STRINGS.unsupportedFile);
    } else {
      api.addSystemNote(STRINGS.fileReadFailed);
    }
  }

  function readPickedFiles(files: File[]): void {
    let newNames = 0;
    for (let i = 0; i < files.length; i++) {
      let alreadyAttached = false;
      for (let j = 0; j < attachments.length; j++) {
        if (sameFileName(attachments[j].name, files[i].name)) {
          alreadyAttached = true;
        }
      }
      if (!alreadyAttached) newNames++;
    }
    if (attachments.length + newNames > MAX_ATTACHED_FILES) {
  ...
  fileInput.onchange = function () {
    ...
    fileInput.value = '';
    if (picked.length > 0) readPickedFiles(picked);
  };
  ...
  function submit(): void {
    ...
    input.value = '';
    // A token deleted by hand intentionally omits that file from the frame.
    const included: LocalFileAttachment[] = [];
    for (let i = 0; i < attachments.length; i++) {
      const att = attachments[i];
      if (text.indexOf(attachmentReference(att.name)) >= 0) included.push(att);
    }
    attachments.length = 0;
    renderAttachments();
    api.addUserMessage(text);
    api.setBusy(true);
    callbacks.onSend(text, included);
  }
