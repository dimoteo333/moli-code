...
import com.alibaba.moli.code.cli.MoliCodeCli;
...
import com.alibaba.moli.code.cli.protocol.data.behavior.Behavior.Operation;
...
import com.alibaba.moli.code.cli.session.event.consumers.AssistantContentSimpleConsumers;
import com.alibaba.moli.code.cli.session.event.consumers.SessionEventConsumers;
import com.alibaba.moli.code.cli.session.event.consumers.SessionEventSimpleConsumers;
...
class SessionTest {
    ...
    void partialSendPromptSuccessfully() throws SessionControlException, SessionSendPromptException {
        Session session = MoliCodeCli.newSession(new TransportOptions().setIncludePartialMessages(true));
    ...
    void setPermissionModeSuccessfully() throws SessionControlException, SessionSendPromptException {
        Session session = MoliCodeCli.newSession(new TransportOptions());
    ...
    void sendPromptAndSetModelSuccessfully() throws SessionControlException, SessionSendPromptException {
        ...
        log.info(session.setModel("moli3-coder-flash").map(s -> s ? "setModel 1 success" : "setModel 1 error").orElse("setModel 1 unknown"));
        writeSplitLine("setModel 1 end");

        session.sendPrompt("hello world", new SessionEventSimpleConsumers());
        writeSplitLine("prompt 1 end");

        log.info(session.setModel("moli3-coder-plus").map(s -> s ? "setModel 2 success" : "setModel 2 error").orElse("setModel 2 unknown"));
        writeSplitLine("setModel 1 end");
        ...
        log.info(session.setModel("moli3-max").map(s -> s ? "setModel 3 success" : "setModel 3 error").orElse("setModel 3 unknown"));
        writeSplitLine("setModel 1 end");
    ...
    void sendPromptAndInterruptContinueSuccessfully() throws SessionControlException, SessionSendPromptException {
        Session session = MoliCodeCli.newSession();
    ...
    void testJSON() {
        String json
                ...
                + " Moli Code SDK for Java today?\",\"type\":\"text\"}],\"id\":\"ed8374fe-a4eb-4fc0-9780-9bd2fd831cda\","
                + "\"model\":\"moli3-coder-plus\",\"role\":\"assistant\",\"type\":\"message\",\"usage\":{\"cache_read_input_tokens\":12766,"
