...
The Moli Code Java SDK is a minimum experimental SDK for programmatic access to Moli Code functionality. It provides a Java interface to interact with the Moli Code CLI, allowing developers to integrate Moli Code capabilities into their Java applications.

**Context Information:**

- Current Date: Monday 5 January 2026
- Operating System: darwin
- Working Directory: /Users/weigeng/repos/moli-code/packages/sdk-java

## Project Details

- **Group ID**: com.alibaba
- **Artifact ID**: molicode-sdk (as per pom.xml)
...
- **API Layer**: Provides the main entry points through `MoliCodeCli` class with simple static methods for basic usage
- **Session Layer**: Manages communication sessions with the Moli Code CLI through the `Session` class
...
- `MoliCodeCli`: Main entry point with static methods for simple queries
...
The project includes basic unit tests using JUnit 5. The main test class `MoliCodeCliTest` demonstrates how to use the SDK to make simple queries to the Moli Code CLI.
...
- **Session creation**: Use `MoliCodeCli.newSession()` to create a new session with custom options
...
The SDK includes several example files in `src/test/java/com/alibaba/moli/code/cli/example/` that demonstrate different aspects of the API:
