...
(IS_SANDBOX ? describe.skip : describe)('acp integration', () => {
  ...
  it('returns modes on initialize and allows setting mode and model', async () => {
    ...
    try {
      ...
      // Test 7: Set model using openai model instead of first available model (index=0) which could be moli-oauth requiring login
  ...
  it('includes authMethods in error data when auth is required', async () => {
    ...
    try {
      ...
      // Attempt to set the first model (which might be moli-oauth requiring login) without authenticating
