...
const IS_SANDBOX =
  process.env['MOLI_SANDBOX'] &&
  process.env['MOLI_SANDBOX']!.toLowerCase() !== 'false';
...
(IS_SANDBOX ? describe.skip : describe)('acp integration', () => {
  ...
  it('returns internal error details when model auth is required', async () => {
    ...
    try {
      ...
      // Choose a moli-oauth model to trigger auth-required path deterministically.
      const moliOauthModel = newSession.models.availableModels.find((model) =>
        model.modelId.includes('moli-oauth'),
      );
      expect(moliOauthModel).toBeDefined();
      await expect(
        sendRequest('session/set_config_option', {
          sessionId: newSession.sessionId,
          configId: 'model',
          value: moliOauthModel!.modelId,
        }),
  ...
  it('receives available_commands_update with slash commands after session creation', async () => {
    ...
    try {
      ...
      // multi-step process (listing files, reading up to 10 files, generating MOLI.md)
