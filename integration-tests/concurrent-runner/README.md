# Moli Concurrent Runner

A Python tool for executing multiple Moli CLI tasks across different models concurrently using isolated git worktrees.
...
| `worktree_base` | string | ~/.moli/worktrees | Base directory for git worktrees              |
  ...
  "runs": [
    {
      ...
      "status": "succeeded",
      "worktree_path": "~/.moli/worktrees/run-abc123",
...
4. **Execute**: Run moli CLI with captured output (logs go to run-specific folder)
