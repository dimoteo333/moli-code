#!/usr/bin/env python3
"""
Moli Concurrent Runner - Execute multiple CLI tasks across different models concurrently.

This tool creates isolated git worktrees for each task/model combination and executes
the Moli CLI in parallel with status tracking and output capture.
"""
...
class RunConfig:
    ...
    worktree_base: Path = field(default_factory=lambda: Path.home() / ".moli" / "worktrees")
...
class RunRecord:
    ...
    model: str
    status: RunStatus
    auth_type: Optional[str] = None  # e.g. "anthropic" for moli --auth-type
...
class GitWorktreeManager:
    ...
    async def collect_session_log(self, worktree_dir: Path, output_dir: Path) -> Optional[Tuple[Path, str, Path]]:
        ...
        ~/.moli/projects/{projectId}/chats/{sessionId}.jsonl
        ...
        moli_dir = Path.home() / ".moli"
        chats_dir = moli_dir / "projects" / project_id / "chats"
...
<head>
    <meta charset="UTF-8">
    <title>Moli Runner Report</title>
    <style>
...
<body>
    <h1>Moli Runner Execution Report</h1>
    <div class="summary">
...
class MoliRunner:
    """Executes the Moli CLI for a specific task and model."""
    ...
    async def run(
        ...
        """Execute the Moli CLI for each prompt sequentially."""
        ...
        for prompt_index, prompt_text in enumerate(task.prompts, start=1):
            ...
            env["MOLI_CODE_ROOT"] = str(worktree_dir_resolved)
    ...
    def _build_command(self, run: RunRecord, prompt_text: str, use_continue: bool = False) -> List[str]:
        """Build the moli CLI command for a single prompt."""
        cmd = ["moli"]
...
def load_config(config_path: Path) -> RunConfig:
    ...
    return RunConfig(
        ...
        worktree_base=Path(data.get("worktree_base", "~/.moli/worktrees")).expanduser(),
...
async def execute_single_run(
    ...
    try:
        ...
        await moli_runner.run(run, worktree_dir, output_dir)
...
async def run_all(config: RunConfig, console: Console) -> ExecutionState:
    ...
    moli_runner = MoliRunner(config, console)
    ...
    async def execute_with_limit(run: RunRecord):
        async with semaphore:
            await execute_single_run(
                run, config, tracker, worktree_manager, moli_runner, console
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Moli Concurrent Runner - Execute multiple CLI tasks across models"
    )
    parser.add_argument(
        "config",
        type=Path,
