# Moli Code Hooks Documentation

## Overview

Moli Code hooks provide a powerful mechanism for extending and customizing the behavior of the Moli Code application. Hooks allow users to execute custom scripts or programs at specific points in the application lifecycle, such as before tool execution, after tool execution, at session start/end, and during other key events.
...
Hooks are user-defined scripts or programs that are automatically executed by Moli Code at predefined points in the application flow. They allow users to:
...
The Moli Code hook system consists of several key components:
...
The following table lists all available hook events in Moli Code:
...
{
  "decision": "block",
  "reason": "Must be provided when Moli Code is blocked from stopping"
}
```
...
Hooks are configured in Moli Code settings, typically in `.qwen/settings.json` or user configuration files:
