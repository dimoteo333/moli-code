# Moli Code Extensions

Moli Code extensions package prompts, MCP servers, subagents, skills and custom commands into a familiar and user-friendly format. With extensions, you can expand the capabilities of Moli Code and share those capabilities with others. They are designed to be easily installable and shareable.

Extensions and plugins from [Gemini CLI Extensions Gallery](https://geminicli.com/extensions/) and [Claude Code Marketplace](https://claudemarketplaces.com/) can be directly installed into Moli Code. This cross-platform compatibility gives you access to a rich ecosystem of extensions and plugins, dramatically expanding Moli Code's capabilities without requiring extension authors to maintain separate versions.
...
This command opens the respective marketplace in your default browser, allowing you to discover new extensions to enhance your Moli Code experience.

> **Cross-Platform Compatibility**: This allows you to leverage the rich extension ecosystems from both Gemini CLI and Claude Code, dramatically expanding the available functionality for Moli Code users.
...
For extensions installed from a local path or a git repository, you can explicitly update to the latest version (as reflected in the `moli-extension.json` `version` field) with `qwen extensions update extension-name`.
...
- `contextFileName`: The name of the file that contains the context for the extension. This will be used to load the context from the extension directory. If this property is not used but a `MOLI.md` file is present in your extension directory, then that file will be loaded.
...
- **User level** (default): Settings apply across all projects (`~/.moli/.env`)
- **Workspace level**: Settings apply only to the current project (`.moli/.env`)
...
When Moli Code starts, it loads all the extensions and merges their configurations. If there are any conflicts, the workspace configuration takes precedence.
...
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `${extensionPath}`         | The fully-qualified path of the extension in the user's filesystem e.g., '/Users/username/.moli/extensions/example-extension'. This will not unwrap symlinks. |
