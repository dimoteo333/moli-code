...
> External messaging integrations for Moli Code — interact with an agent from Telegram, WeChat, and more.
>
> User documentation: [Channels Overview](../../users/features/channels/overview.md).

## Overview

A **channel** connects an external messaging platform to a Moli Code agent. Configured in `settings.json`, managed via `qwen channel` subcommands, multi-user (each user gets an isolated ACP session).
...
One `moli-code --acp` process with multiple ACP sessions. Scope per channel: **`user`** (default), **`thread`**, or **`single`**. Routing keys namespaced as `<channelName>:<key>`.
...
```
packages/channels/
├── base/                    # @dobby/moli-code-channel-base
│   └── src/
...
├── telegram/                # @dobby/moli-code-channel-telegram
├── weixin/                  # @dobby/moli-code-channel-weixin
└── dingtalk/                # @dobby/moli-code-channel-dingtalk
```
