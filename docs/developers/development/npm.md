...
Releases are managed through the [release.yml](https://github.com/dimoteo333/moli-code/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
1.  Go to the [Actions tab](https://github.com/dimoteo333/moli-code/actions/workflows/release.yml) of the repository.
...
You can then inspect the generated tarballs to ensure that they contain the correct files and that the `package.json` files have been updated correctly. The tarballs will be created in the root of each package's directory (e.g., `packages/cli/moli-code-0.1.6.tgz`).
