# Moli Code file system tools

Moli Code provides a comprehensive suite of tools for interacting with the local file system. These tools allow the model to read from, write to, list, search, and modify files and directories, all under your control and typically with confirmation for sensitive operations.
...
When reading files, Moli Code detects the file's encoding using a multi-step strategy:
...
These file system tools provide a foundation for Moli Code to understand and interact with your local project context.
