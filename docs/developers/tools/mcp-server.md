# MCP servers with Moli Code

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with Moli Code.
...
Moli Code integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
Moli Code uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
Moli Code supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
- **Stored securely** in `~/.moli/mcp-oauth-tokens.json`
...
When Moli Code starts, it performs MCP server discovery through the following detailed process:
...
When Moli Code receives this response, it will:
...
In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within Moli Code. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="Moli Code" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Moli Code" reverent
```
