...
This document describes the `task` tool for Moli Code.
...
## How to use `task` with Moli Code
...
You can view available subagents by using the `/agents` command in Moli Code.
...
Subagents are configured through Moli Code's agent configuration system. Use the `/agents` command to:
