...
This document describes the `todo_write` tool for Moli Code.
...
## How to use `todo_write` with Moli Code
...
The tool stores todo lists in your home directory (`~/.moli/todos/`) with session-specific files, so each coding session maintains its own task list.
