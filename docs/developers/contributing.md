...
```bash
git clone https://github.com/dimoteo333/moli-code.git # Or your fork's URL
cd moli-code
```
...
To start the Moli Code application from the source code (after building), run the following command from the root directory:

```bash
npm start
```

If you'd like to run the source build outside of the moli-code folder, you can utilize `npm link path/to/moli-code/packages/cli` (see: [docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) to run with `moli-code`
...
The integration tests are designed to validate the end-to-end functionality of Moli Code. They are not run as part of the default `npm run test` command.
...
- `packages/`: Contains the individual sub-packages of the project.
  ...
  - `core/`: The core backend logic for Moli Code.
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect moli-code due to automatic exclusion. Use `.moli-code/.env` files for moli-code specific debug settings.

### React DevTools

To debug the CLI's React-based UI, you can use React DevTools. Ink, the library used for the CLI's interface, is compatible with React DevTools version 4.x.

1.  **Start the Moli Code application in development mode:**
